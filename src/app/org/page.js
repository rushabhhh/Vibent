    ...
    <div className="min-h-screen bg-[#0B0B0F] text-white">
      ...
      <section id="templates" className="mx-auto max-w-7xl px-6 pb-12 pt-8 md:pt-12">
        <div className="mx-auto max-w-2xl text-center">
          ...
          <p className="mt-3 text-white/70">Prebuilt, customizable, and verifiable on-chain.</p>
        </div>

        <div className="mt-10 grid gap-6 md:grid-cols-2 lg:grid-cols-3">
          {templates.map((t) => (
            <div key={t.title} className="relative overflow-hidden rounded-2xl border border-white/10 bg-gradient-to-br from-violet-900/20 via-fuchsia-900/20 to-indigo-900/20 p-6">
              <div className="text-lg font-semibold">{t.title}</div>
              <div className="mt-1 text-xs text-white/60">{t.tag}</div>
              <div className="mt-3 inline-flex items-center gap-2 rounded-md border border-white/15 bg-white/10 px-2 py-1 text-[11px]">
                Type: {t.type}
              </div>
              <div className="pointer-events-none absolute -right-10 -top-10 h-40 w-40 rounded-full bg-fuchsia-500/20 blur-3xl" />
              <div className="mt-5">
                <Link href="/org/auth" className="text-sm text-white/80 hover:text-white">Use this template →</Link>
              </div>
            </div>
          ))}
        </div>
      ...
      <section id="explorer" className="mx-auto max-w-7xl px-6 pb-12 pt-8 md:pt-12">
        <div className="mx-auto max-w-2xl text-center">
          ...
          <p className="mt-3 text-white/70">
            Filter by Proof of Talent, Proof of Vibe, skills, and endorsements.
          </p>
        </div>
        ...
        <div className="mx-auto mt-8 max-w-4xl rounded-2xl border border-white/10 bg-white/5 p-6">
          <div className="flex flex-wrap items-center gap-2 text-xs">
            {filters.map((f) => (
              <span key={f} className="rounded-lg border border-white/10 bg-white/5 px-3 py-1.5">{f}</span>
            ))}
            <span className="ml-auto rounded-lg border border-white/10 bg-black/30 px-3 py-1.5">BNB Chain</span>
          </div>

          <div className="mt-4 space-y-3 text-sm">
            {[
              { name: 'Aria N.', pot: 84, pov: 79, skills: 'Solidity, zk, Next.js' },
              { name: 'Devon P.', pot: 80, pov: 77, skills: 'Graph ML, Rust, Vector DB' },
              { name: 'Mira O.', pot: 76, pov: 82, skills: 'Product, Community, Design' },
            ].map((t) => (
              <div key={t.name} className="flex items-center gap-3 rounded-xl border border-white/10 bg-black/30 p-3">
                ...
                <div className="min-w-0 flex-1">
                  <div className="flex items-center justify-between">
                    ...
                    <div className="flex items-center gap-2 text-xs">
                      <span className="rounded-md border border-white/10 bg-white/5 px-2 py-0.5">PoT {t.pot}</span>
                      <span className="rounded-md border border-white/10 bg-white/5 px-2 py-0.5">PoV {t.pov}</span>
                    </div>
                  </div>
                  <div className="truncate text-white/60">{t.skills}</div>
                </div>
                <Link href="/org/auth" className="rounded-md border border-white/10 bg-white/5 px-2.5 py-1 text-xs hover:bg-white/10">
                  Save to watchlist
                </Link>
              </div>
            ))}
